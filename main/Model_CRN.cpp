...
void Model_CRN::LoadState(const char *state_path) {
    ...
    enc_bn1.LoadState(pMatFile, "conv1_bn");
    enc_bn2.LoadState(pMatFile, "conv2_bn");
    enc_bn3.LoadState(pMatFile, "conv3_bn");
    enc_bn4.LoadState(pMatFile, "conv4_bn");
    enc_bn5.LoadState(pMatFile, "conv5_bn");
    dec_bn5.LoadState(pMatFile, "conv5_t_bn");
    dec_bn4.LoadState(pMatFile, "conv4_t_bn");
    dec_bn3.LoadState(pMatFile, "conv3_t_bn");
    dec_bn2.LoadState(pMatFile, "conv2_t_bn");
...
void Model_CRN::sprint(Eigen::Tensor<float_t, 4> input) {
    const Eigen::Tensor<size_t, 4>::Dimensions &dim_inp = input.dimensions();
    std::cout << "Variable:" << std::endl;
    // 0 0
    std::cout << input(0, 0, 0, 0) << " " << input(0, 0, 0, 1) << " " << input(0, 0, 0, 2) << " ";
    std::cout << input(0, 0, 0, dim_inp[3] - 3) << " " << input(0, 0, 0, dim_inp[3] - 2) << " "
              << input(0, 0, 0, dim_inp[3] - 1);
    std::cout << std::endl;

    // 0 4
    std::cout << input(0, 0, 4, 0) << " " << input(0, 0, 4, 1) << " " << input(0, 0, 4, 2) << " ";
    std::cout << input(0, 0, 4, dim_inp[3] - 3) << " " << input(0, 0, 4, dim_inp[3] - 2) << " "
              << input(0, 0, 4, dim_inp[3] - 1);
    std::cout << std::endl;


    // 0 -1
    std::cout << input(0, 0, dim_inp[2] - 1, 0) << " " << input(0, 0, dim_inp[2] - 1, 1) << " "
              << input(0, 0, dim_inp[2] - 1, 2) << " ";
    std::cout << input(0, 0, dim_inp[2] - 1, dim_inp[3] - 3) << " " << input(0, 0, dim_inp[2] - 1, dim_inp[3] - 2)
              << " "
...
Eigen::Tensor<float_t, 4> Model_CRN::forward(Eigen::Tensor<float_t, 4> &inp) {
    ...
    e5 = this->ac.ELU(e5);
    std::cout << "Encoder takes：" << (GetTickCount() - star_time) << "ms." << std::endl;
    ...
    sprint(lstm_out_shuffle);
    std::cout << "Sequence takes：" << (GetTickCount() - star_time) << "ms." << std::endl;
    star_time = GetTickCount();
    ...
    std::cout << "Decoder takes：" << (GetTickCount() - star_time) << "ms." << std::endl;
    star_time = GetTickCount();
